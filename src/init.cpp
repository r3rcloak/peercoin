 ...
 * between cloakcoind, and cloakcoin-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("cloakcoin-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/CloakProject/>";
    const std::string URL_WEBSITE = "<https://cloakcoin.com/>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("cloakcoin-loadblk");
...
/** Sanity checks
 *  Ensure that Cloakcoin is running in a usable environment with all
...
void InitLogging()
{
    fPrintToConsole = gArgs.GetBoolArg("-printtoconsole", true);  // cloak - always default to printing to console as per btc 0.17.x
...
{
    // Make sure only a single Cloakcoin process is using the data directory.
...
bool AppInitSanityChecks()
{
    ...
    // cloakcoin: init hash seed
    cloakcoinRandseed = GetRand(1 << 30);

#ifdef ENABLE_CHECKPOINTS
    // cloakcoin: moved here because ECC need to be initialized to execute this
    if (gArgs.IsArgSet("-checkpointkey")) // cloakcoin: checkpoint master priv key
    {
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if cloakcoin is started in the future "
...
#ifdef ENABLE_CHECKPOINTS
                // cloakcoin: initialize synchronized checkpoint
                if (!fReindex && !WriteSyncCheckpoint(chainparams.GenesisBlock().GetHash()))
                    return error("LoadBlockIndex() : failed to init sync checkpoint");

                // cloakcoin: if checkpoint master key changed must reset sync-checkpoint
